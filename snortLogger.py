...
class Rule:
    def __init__(self,rule_id, action,priority, protocol, source_ip, source_port, dest_ip, dest_port, message):
        self.protocolDict = {
            "tcp": 6,
            "icmp": 1,
            "udp": 17,
        ...
        ni.ifaddresses('wlo1')
        local_ip = ni.ifaddresses('wlo1')[ni.AF_INET][0]['addr']
        
        self.rule_id = rule_id
        self.action = action 
        self.priority = priority
        self.protocol = protocol
        self.source_ip = local_ip if source_ip == "home" else source_ip  
        self.source_port = source_port
        self.dest_ip = local_ip if dest_ip == "home" else dest_ip
        self.dest_port = dest_port 
        self.message = message
        print("source_ip: ",self.source_ip)
    ...
    def fromJson(json):
        return Rule(json["rule_id"],json["action"],json["priority"], json["protocol"], json["source_ip"], json["source_port"], json["dest_ip"], json["dest_port"], json["message"])
...
fileName = f"snort-log-{str(datetime.now().strftime('%m-%d-%Y-%H:%M:%S'))}.log"
# receive a packetS
while True:
    ...
    if eth_protocol == 8 :
        ...
        mac_address_route = f"{hex(eth_unpacked[0][0])[2:]}:{hex(eth_unpacked[0][1])[2:]}:{hex(eth_unpacked[0][2])[2:]}:{hex(eth_unpacked[0][3])[2:]}:{hex(eth_unpacked[0][4])[2:]}:{hex(eth_unpacked[0][5])[2:]} ---> {hex(eth_unpacked[0][0])[2:]}:{hex(eth_unpacked[1][1])[2:]}:{hex(eth_unpacked[1][2])[2:]}:{hex(eth_unpacked[1][3])[2:]}:{hex(eth_unpacked[1][4])[2:]}:{hex(eth_unpacked[1][5])[2:]}"
        ...
        if protocol == 6 :
            ...
            for rule in rules:
                if (rule.matches("tcp", source_ip, source_port, dest_ip, dest_port)):
                    ...
                    print(hex(eth_unpacked[0][0])[2:],":",hex(eth_unpacked[0][1])[2:],":",hex(eth_unpacked[0][2])[2:],":",hex(eth_unpacked[0][3])[2:],":",hex(eth_unpacked[0][4])[2:],":",hex(eth_unpacked[0][5])[2:],end=" ----> ")
                    print(hex(eth_unpacked[1][0])[2:],":",hex(eth_unpacked[1][1])[2:],":",hex(eth_unpacked[1][2])[2:],":",hex(eth_unpacked[1][3])[2:],":",hex(eth_unpacked[1][4])[2:],":",hex(eth_unpacked[1][5])[2:])

                    
                    log_file.write(f"[**] [{rule.rule_id}]  {rule.message} [**]\n[Priority: {rule.priority}]\n{datetime.now().strftime('%m-%d-%Y-%H:%M:%S')}\n {rule.protocol} {source_ip}:{source_port} -> {dest_ip}:{dest_port}\n")
        ...
        #ICMP Packets
            ...
            for rule in rules:
                if (rule.matches("icmp", source_ip,"any", dest_ip,"any")):
                    ...
                    print(hex(eth_unpacked[0][0])[2:],":",hex(eth_unpacked[0][1])[2:],":",hex(eth_unpacked[0][2])[2:],":",hex(eth_unpacked[0][3])[2:],":",hex(eth_unpacked[0][4])[2:],":",hex(eth_unpacked[0][5])[2:],end=" ----> ")
                    print(hex(eth_unpacked[1][0])[2:],":",hex(eth_unpacked[1][1])[2:],":",hex(eth_unpacked[1][2])[2:],":",hex(eth_unpacked[1][3])[2:],":",hex(eth_unpacked[1][4])[2:],":",hex(eth_unpacked[1][5])[2:])
                    log_file.write(f"[**] [{rule.rule_id}]  {rule.message} [**]\n[Priority: {rule.priority}]\n{datetime.now().strftime('%m-%d-%Y-%H:%M:%S')}\n {rule.protocol} {source_ip}:{source_port} -> {dest_ip}:{dest_port}\n"+'ICMP type : ' + str(icmp_type)+"ICMP code : "+str(icmp_code))
